...
const Footprint: NextPage = () => {
    ...
    return (
      <div className="space-y-8">
        <div className="bg-white p-8 rounded-2xl shadow-md mobile-enhanced">
          <h1 className="text-3xl font-semibold text-gray-900 mb-2 tracking-tight">Environmental Footprint</h1>
          <p className="text-gray-600 mb-8 max-w-2xl leading-relaxed text-base text-enhanced">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 mb-10">
              {plants.map(plant => {
                ...
                return co2 !== null ? (
                  <div
                    key={plant.id}
                    className="bg-gradient-to-br from-white to-gray-50 p-5 rounded-xl shadow-sm border border-gray-100 flex flex-col gap-3 hover:shadow-lg transition-shadow group card-hover"
                  >
                    ...
                    <div className="flex items-center gap-2 text-gray-700">
                      <span className="text-sm">Water Saved: <span className="font-medium text-water">{water} L</span></span>
                    </div>
                  </div>
                ) : null;
              })}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
            <div className="flex flex-col items-start bg-green-50 p-6 rounded-xl shadow-sm border border-green-100 card-hover">
            ...
            <div className="flex flex-col items-start bg-blue-50 p-6 rounded-xl shadow-sm border border-blue-100 card-hover">
              <h2 className="text-lg font-semibold text-gray-900 mb-1">Car Miles</h2>
              <p className="text-xs text-gray-500 mb-1">Avg CO₂ emitted per mile</p>
              <p className="text-2xl font-bold text-primary">{(totalCO2 / gramsPerMile).toFixed(2)} mi</p>
            </div>
            <div className="flex flex-col items-start bg-yellow-50 p-6 rounded-xl shadow-sm border border-yellow-100 card-hover">
              <h2 className="text-lg font-semibold text-gray-900 mb-1">Tree Equivalent</h2>
              <p className="text-xs text-gray-500 mb-1">Avg CO₂ absorbed by 1 tree/year</p>
              <p className="text-2xl font-bold text-primary">{(totalCO2 / gramsPerTree).toFixed(2)} trees</p>
            </div>
            <div className="flex flex-col items-start bg-cyan-50 p-6 rounded-xl shadow-sm border border-cyan-100 card-hover">
              <h2 className="text-lg font-semibold text-gray-900 mb-1">Total Water Saved</h2>
              <p className="text-2xl font-bold text-water">{totalWaterUse.toLocaleString()} L</p>
            </div>
          </div>
        </div>
      </div>
