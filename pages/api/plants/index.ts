...
export let plants: Plant[] = [
  {
    ...
    description: 'Also known as the Swiss Cheese Plant, featuring large, heart-shaped leaves with distinctive splits and holes.',
    sensorId: 'sensor-001',
    carbonSavings: 120,
  },
  {
    ...
    location: 'Bedroom',
    imageUrl: 'https://images.unsplash.com/photo-1593691723820-4d20eb2a2b56?ixlib=rb-1.2.1&auto=format&fit=crop&w=800&q=80',
  ...
  },
  {
    ...
    description: 'A hardy, drought-resistant succulent with tall, stiff leaves. Excellent for beginners.',
    carbonSavings: 95,
  },
];
...
function handleGetPlants(req: NextApiRequest, res: NextApiResponse<PlantsApiResponse>) {
  // In a real app, this would have filtering, pagination, etc.
  res.status(200).json({ success: true, data: plants });
}

function handleAddPlant(req: NextApiRequest, res: NextApiResponse<PlantApiResponse>) {
  try {
    ...
    if (!newPlant.name || !newPlant.species || !newPlant.location) {
      return res.status(400).json({ 
        success: false, 
        error: 'Missing required fields: name, species, and location are required' 
      });
    }
