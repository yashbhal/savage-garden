...
const generateMockReadings = (plantId: string): SensorReading[] => {
  ...
  const idSum = plantId.split('').reduce((sum, char) => sum + char.charCodeAt(0), 0);
  ...
  for (let day = 29; day >= 0; day--) {
    for (let hour = 0; hour < 24; hour += 6) {
      ...
      readings.push({
        ...
        moisture: Math.min(100, Math.max(0, Math.floor(moistureBase + dayVariance + randomVariance()))),
        temperature: Math.floor((temperatureBase + hourVariance / 2 + randomVariance() / 2) * 10) / 10,
        light: Math.floor(lightBase + hourVariance * 50 + dayVariance * 30 + randomVariance() * 20),
        weight: Math.floor(weightBase + dayVariance / 5 + hourVariance / 10 + randomVariance()),
      });
    }
  }
